...
// ссылку тем же WS-каналом (`call_link`). Сервер хранит её в комнате (feat.9)
// и доносит при подключении — в том числе если отправил, пока ученик был
// офлайн. Показываем как есть, без валидации содержимого (ссылку прислал
// тьютор из своего личного кабинета, не произвольный ввод извне).
...
// feat.9: клик по трубке = ученик принял приглашение. Сервер сам отзывает
// ссылку (чистит pending комнаты и говорит тьютору «Принято»), значок прячем
// сразу — приглашение израсходовано, F5 его уже не вернёт.
callLinkIcon.addEventListener('click', () => {
  sendMessage({type: 'call_link_accepted'});
  hideCallLink();
});
