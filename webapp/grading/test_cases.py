...
TEST_CASES = {
    ...
    # 101 — служебная тестовая задача (feat.2): на ней тьютор отрабатывает
    # откат решённой задачи. Видна только тьютору (см. HIDDEN_TASK_IDS ниже и
    # фильтр по роли в webapp/routes/tasks_api.py:list_tasks).
    101: [
        [1, 2, 3],
        [10, -5, 5],
        [42],
...
# Задания, скрытые от ученика в веб-грейдере (видит только тьютор). Сюда
# попадают служебные задачи — например, тестовые, на которых тьютор
# отрабатывает механику, не трогая реальные попытки. Как и TEST_CASES, это
# курируемый набор: задача доступна iff она в TEST_CASES; задача скрыта от
# ученика iff она дополнительно в HIDDEN_TASK_IDS.
HIDDEN_TASK_IDS: set[int] = {101}
